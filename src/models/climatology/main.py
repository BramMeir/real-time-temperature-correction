"""
Main script for the hourly climatology baseline model.

Example usage:
python -m src.models.climatology.main --input_file ./data/preprocessed.csv
"""
import argparse
import numpy as np
import pandas as pd
from src.models.climatology.execute_forecast import run_single_forecast


def repeat_task(df, target_station, exog_cols, random_seed=42, n_repeats=10, weeks=8, hours_to_forecast=48):
    """
    Repeats the training and evaluation of the hourly climatology model over random training/forecast periods.

    Input
    -----
    df: DataFrame with time series data, indexed by datetime
    target_station: Name of the target station column to predict
    exog_cols: List of additional station columns, unused by the climatology (kept for interface compatibility)
    ...
    """
    # Select random n_repeats parts with `weeks` training and `hours_to_forecast` test from the df
    ...
    for (start, train_end, end) in date_ranges:
        mae, mse, best_hp = run_single_forecast(df, target_station, exog_cols=exog_cols,
                                                 start=start, train_end=train_end, test_end=end)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Hourly Climatology Baseline Script")
    ...
    # Define the other stations as exogenous variables (unused by the climatology)
